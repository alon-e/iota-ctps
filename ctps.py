...
class tangle:
    ...
    def add_stats(self):
        ...
        self.broadcast_data([self.prev_timestamp, num_txs, num_ctxs, '{:.1f}'.format(100 * num_ctxs / (num_txs * 1.0)), '{:.0f}'.format(tps), '{:.0f}'.format(ctps),width, avg_c_t])
    ...
    def broadcast_data(self, data):
        if self.broadcast_max_tps < int(data[4]):
            self.broadcast_max_tps = int(data[4])
        if self.broadcast_max_ctps < int(data[5]):
            self.broadcast_max_ctps = int(data[5])
